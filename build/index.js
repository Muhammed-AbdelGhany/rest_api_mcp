...
if (!BASE_URL)
    throw new Error("REST_BASE_URL env var is required");
let tokenCache = null;
...
async function discoverLoginEndpoint() {
    if (LOGIN_ENDPOINT_OVERRIDE)
        return LOGIN_ENDPOINT_OVERRIDE;
...
function detectTokenPath(responseData) {
    if (TOKEN_PATH_OVERRIDE)
        return TOKEN_PATH_OVERRIDE;
    if (discoveredTokenPath)
        return discoveredTokenPath;
...
async function login() {
    if (!EMAIL || !PASSWORD)
        throw new Error("API_EMAIL and API_PASSWORD env vars are required for authenticated requests");
    ...
    if (LOGIN_CREDENTIALS_JSON) {
        try {
            Object.assign(loginBody, JSON.parse(LOGIN_CREDENTIALS_JSON));
        }
        catch {
            throw new Error(`API_LOGIN_CREDENTIALS is not valid JSON: ${LOGIN_CREDENTIALS_JSON}`);
        }
    }
    ...
    if (VERIFY_ENDPOINT) {
        ...
        for (const field of SESSION_FIELD_CANDIDATES) {
            const val = getNestedValue(step1.data, field) ??
        ...
        if (VERIFY_EXTRA_JSON) {
            try {
                Object.assign(verifyBody, JSON.parse(VERIFY_EXTRA_JSON));
            }
            catch {
                throw new Error(`API_VERIFY_CREDENTIALS is not valid JSON: ${VERIFY_EXTRA_JSON}`);
            }
        }
    ...
    if (!tokenPath) {
        throw new Error(`Could not find token in ${VERIFY_ENDPOINT ? "verify" : "login"} response. ` +
            `Tried paths: ${TOKEN_PATH_CANDIDATES.join(", ")}. ` +
            `Response: ${JSON.stringify(finalData).slice(0, 500)}. ` +
            `Set API_TOKEN_PATH env var explicitly.`);
    }
    const token = getNestedValue(finalData, tokenPath);
    const cache = {
        token,
...
// ── MCP Server ─────────────────────────────────────────────────────────────────
const server = new Server({ name: "rest-api-mcp", version: "1.0.0" }, { capabilities: { tools: {}, resources: {} } });
// ── Resources ──────────────────────────────────────────────────────────────────
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        },
        {
            ...
            description: "Fuzzy-search the API spec by keyword. Use this when you don't know the exact path. " +
        ...
        {
            name: "request",
            description: "Makes an authenticated API call. Handles login automatically — if the token is expired it re-logins transparently. Returns the full response body plus login_data (which contains IDs like pharmacyId returned from login).",
            inputSchema: {
                type: "object",
                properties: {
                    method: {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    if (name === "search_endpoints") {
        ...
        try {
            ...
            const lines = top.map((h, i) => `${i + 1}. ${h.method} ${h.path}` +
                (h.summary ? `\n   Summary: ${h.summary}` : "") +
                (h.tags.length ? `\n   Tags: ${h.tags.join(", ")}` : "") +
    ...
    // ── fetch_spec ──────────────────────────────────────────────────────────────
    if (name === "fetch_spec") {
        const specUrl = args?.url ?? SWAGGER_URL;
        if (!specUrl) {
    ...
    if (name === "request") {
        ...
        const skipAuth = args?.skip_auth ?? false;
        let authHeaders = {};
        let loginData = {};
        if (!skipAuth) {
            try {
